...
## 5-1. 미니게임 추가 / 교체 방법
직원 참여형 게임은 주기적으로 교체하는 것을 전제로 만들어져 있습니다.

- **점수 저장**: 게임마다 테이블을 만들지 않고 공통 테이블 `game_scores`(`game_key` 컬럼으로 구분, 세부기록은 `detail` jsonb)를 씁니다. 최초 1회만 `supabase/game_scores.sql`을 회사 Postgres에 실행하면 되고, **이후 새 게임은 DDL 없이 코드만 추가**하면 됩니다. (하루 1회 제한·점수 상한은 이 SQL의 트리거/CHECK가 서버에서 강제)
  - 단, 새 게임의 점수 상한은 `game_scores_bounds` 제약에 `game_key` 분기를 한 줄 추가해 조여두는 것이 좋습니다.
- **노출 제어**: `index.html`의 `FEATURE_DEFS` 배열에 한 줄(`{key, icon, name, page, nav, card, def}`)만 추가하면 홈 카드·하단 메뉴·관리자 공개 토글·👁️ 미리보기가 자동으로 연결됩니다. 설정값은 `app_settings.feature_visibility`에 저장됩니다.
  - `def`는 기본 공개 여부입니다. **신규 게임은 `def:false`(기본 비공개)로 두고**, DB 준비와 검수를 마친 뒤 관리자 화면에서 공개로 켜세요.
  - 게임을 교체할 때는 이전 게임을 비공개로 내리고 새 게임을 공개로 켭니다. 코드와 기록은 남으므로 언제든 되돌릴 수 있습니다.
- 현재 등록된 게임: `typing`(우주 타자왕, 파일럿), `match`(우주 용어 매치 — 기억력 + 우주산업 용어 학습)
